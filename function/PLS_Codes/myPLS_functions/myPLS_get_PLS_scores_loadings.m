...
for iG = 1:nGroups
    idx = iter:iter + nBehav - 1;
    this_groupID = find(grouping == groupIDs(iG));
    corr_Ly_Y(idx,:) = corr_2015(Ly(this_groupID,:),Y(this_groupID,:))';
    iter = iter + nBehav;
end
...
for iG = 1:nGroups
    idx = iter:iter + nBehav - 1;
    this_groupID = find(grouping == groupIDs(iG));
    corr_Lx_Y(idx,:) = corr_2015(Lx(this_groupID,:),Y(this_groupID,:))';
    iter = iter + nBehav;
end

% Correlations between Ly and X
corr_Ly_X = corr_2015(Ly,X)';
