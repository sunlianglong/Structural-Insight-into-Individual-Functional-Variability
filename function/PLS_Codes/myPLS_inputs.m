function [input,pls_opts,save_opts] = myPLS_inputs(X,Y,nPerms,nBootstraps)
...
input.grouping = ones(length(Y0behav(:,1)),1);
...
name_tmp = {};
for i=1:length(length(Y0behav(1,:)))
    name_tmp{i} = string(i);
end
%input.behav_names = {'age','sex','IQ'}; 
input.behav_names = name_tmp;
% --- Names of the imaging data ---
% will only be used if save_opts.img_type='barPlot'
for ii = 1:length(X0(1,:)); input.img_names{ii,1} = ['img ' num2str(ii)]; end
...
pls_opts.nPerms = nPerms;
pls_opts.nBootstraps = nBootstraps;
...
lenX0 = length(X0(1,:));
input.brain_data = input.brain_data(:,1:lenX0); 
